...
const MyFavs = () => {
    ...
    const favsToRender = favPosts?.map(item => {
        return (
            <PostItem key={item.dataID} hashtags={item.data.hashtags} seconds={item.data.date.seconds} imgPath={item.data.imgPath} likes={item.data.likedBy} id={item.dataID} photo={item.data.userPhotoURl} name={item.data.userName} body={item.data.postBody}/>
        )
    });
